...
int luaO_rawarith(lua_State* L, int op, const TValue* p1, const TValue* p2, TValue* res)
{
    switch (op)
    {
        ...
        case LUA_OPDIV:
        case LUA_OPPOW: { /* operate only on floats */ // TODO
            /*lua_Number n1;
            lua_Number n2;
        ...
        default: { /* other operations */
            ...
            else if (true)//(tonumberns(p1, n1) && tonumberns(p2, n2))
            {
                setfltvalue(res, numarith(L, op, n1, n2));
                return 1;
            }
...
static const char* l_str2dloc(const char* s, lua_Number* result, int mode) // TODO: handle hex
{
    char* endptr;
    *result = lua_Number(s);
    if (endptr == s)
...
size_t luaO_str2num(const char* s, TValue* o)
{
    size_t size;
    lua_Number fp(s);
    if (fp.fractionPart() == lua_Number("0.0"))
    { /* try as an integer */
...
const char* luaO_pushvfstring(lua_State* L, const char* fmt, va_list argp)
{
    ...
    while ((e = strchr(fmt, '%')) != NULL)
    {
        ...
        {               /* conversion specifier */
            ...
            case 'f': { /* a 'lua_Number' */
                TValue num;
                setfltvalue(&num, va_arg(argp, l_uacNumber));
                addnum2buff(&buff, &num);
                break;
            }
            case 'p': {                               /* a pointer */
