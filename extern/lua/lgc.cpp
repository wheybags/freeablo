...
#define markkey(g, n)                                                                                                                                          \
    {                                                                                                                                                          \
        if                                                                                                                                                     \
            keyiswhite(n) reallymarkobject(g, gckey(n));                                                                                                       \
    }
...
void luaC_checkfinalizer(lua_State* L, GCObject* o, Table* mt)
{
    ...
    { /* move 'o' to 'finobj' list */
        ...
        { /* correct pointers into 'allgc' list, if needed */
            if (o == g->survival)
                g->survival = o->next;
            if (o == g->old)
                g->old = o->next;
...
static void incstep(lua_State* L, global_State* g)
{
    ...
    l_mem stepsize =
        (g->gcstepsize <= log2maxs(l_mem)) ? ((cast(l_mem, 1) << g->gcstepsize) / WORK2MEM) * stepmul : MAX_LMEM; /* overflow; keep maximum value */
    do
