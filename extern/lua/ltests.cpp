...
{
    return (badexit("PANIC: unprotected error in call to Lua API (%s)\n", lua_tostring(L, -1), NULL), 0); /* do not return to Lua */
}
...
Memcontrol l_memcontrol = {0UL, 0UL, 0UL, 0UL, (~0UL), {0UL, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL}};
...
static int testobjref1(global_State* g, GCObject* f, GCObject* t)
{
    ...
    { /* generational mode */
        if ((getage(f) == G_OLD && isblack(f)) && !isold(t))
            return 0;
        if (((getage(f) == G_OLD1 || getage(f) == G_TOUCHED2) && isblack(f)) && getage(t) == G_NEW)
            return 0;
        return 1;
    }
}
...
static void checkvalref(global_State* g, GCObject* f, const TValue* t) { lua_assert(!iscollectable(t) || (righttt(t) && testobjref(g, f, gcvalue(t)))); }
...
static void checkobject(global_State* g, GCObject* o, int maybedead, int listage)
{
    ...
    else
    {
        ...
        { /* generational mode? */
            ...
            if (isold(o))
            {
                lua_assert(isblack(o) || getage(o) == G_TOUCHED1 || getage(o) == G_OLD0 || o->tt == LUA_VTHREAD ||
...
static void checklist(global_State* g, int maybedead, int tof, GCObject* newl, GCObject* survival, GCObject* old, GCObject* reallyold)
{
    GCObject* o;
...
static char* buildop(Proto* p, int pc, char* buff)
{
    ...
    {
        case iABC:
            sprintf(buff, "%-12s%4d %4d %4d%s", name, GETARG_A(i), GETARG_B(i), GETARG_C(i), GETARG_k(i) ? " (k)" : "");
            break;
        case iABx:
...
static int gc_color(lua_State* L)
{
    ...
    else
    {
        ...
        lua_pushstring(L, isdead(G(L), obj) ? "dead" : iswhite(obj) ? "white" : isblack(obj) ? "black" : "gray");
    }
    return 1;
}
...
static int gc_age(lua_State* L)
{
    ...
    {
        static const char* gennames[] = {"new", "survival", "old0", "old1", "old", "touched1", "touched2"};
        GCObject* obj = gcvalue(o);
...
{
    static const char* statenames[] = {"propagate", "atomic", "enteratomic", "sweepallgc", "sweepfinobj", "sweeptobefnz", "sweepend", "callfin", "pause", ""};
    static const int states[] = {GCSpropagate, GCSenteratomic, GCSatomic, GCSswpallgc, GCSswpfinobj, GCSswptobefnz, GCSswpend, GCScallfin, GCSpause, -1};
...
{
    static const char* const codes[] = {"OK", "YIELD", "ERRRUN", "ERRSYNTAX", MEMERRMSG, "ERRGCMM", "ERRERR"};
...
static int runC(lua_State* L, lua_State* L1, const char* pc)
{
    ...
    for (;;)
    {
        ...
        if
            ...
            EQ("absindex") { lua_pushnumber(L1, lua_absindex(L1, getindex)); }
        else if
            EQ("append")
            {
                int t = getindex;
            ...
            EQ("concat") { lua_concat(L1, getnum); }
        else if
            EQ("copy")
            {
                int f = getindex;
            ...
            EQ("getglobal") { lua_getglobal(L1, getstring); }
        else if
            EQ("getmetatable")
            {
            ...
            EQ("gettable") { lua_gettable(L1, getindex); }
        else if
            EQ("gettop") { lua_pushinteger(L1, lua_gettop(L1)); }
        else if
            EQ("gsub")
            {
                int a = getnum;
            ...
            EQ("insert") { lua_insert(L1, getnum); }
        else if
            EQ("iscfunction") { lua_pushboolean(L1, lua_iscfunction(L1, getindex)); }
        else if
            EQ("isfunction") { lua_pushboolean(L1, lua_isfunction(L1, getindex)); }
        else if
            EQ("isnil") { lua_pushboolean(L1, lua_isnil(L1, getindex)); }
        else if
            EQ("isnull") { lua_pushboolean(L1, lua_isnone(L1, getindex)); }
        else if
            EQ("isnumber") { lua_pushboolean(L1, lua_isnumber(L1, getindex)); }
        else if
            EQ("isstring") { lua_pushboolean(L1, lua_isstring(L1, getindex)); }
        else if
            EQ("istable") { lua_pushboolean(L1, lua_istable(L1, getindex)); }
        else if
            EQ("isudataval") { lua_pushboolean(L1, lua_islightuserdata(L1, getindex)); }
        else if
            EQ("isuserdata") { lua_pushboolean(L1, lua_isuserdata(L1, getindex)); }
        else if
            EQ("len") { lua_len(L1, getindex); }
        else if
            EQ("Llen") { lua_pushinteger(L1, luaL_len(L1, getindex)); }
        else if
            EQ("loadfile") { luaL_loadfile(L1, luaL_checkstring(L1, getnum)); }
        else if
            EQ("loadstring")
            {
            ...
            EQ("newmetatable") { lua_pushboolean(L1, luaL_newmetatable(L1, getstring)); }
        else if
            EQ("newtable") { lua_newtable(L1); }
        else if
            EQ("newthread") { lua_newthread(L1); }
        else if
            EQ("resetthread") { lua_pushinteger(L1, lua_resetthread(L1)); }
        else if
            EQ("newuserdata") { lua_newuserdata(L1, getnum); }
        else if
            EQ("next") { lua_next(L1, -2); }
        else if
            EQ("objsize") { lua_pushinteger(L1, lua_rawlen(L1, getindex)); }
        else if
            EQ("pcall")
            {
                int narg = getnum;
            ...
            EQ("pushbool") { lua_pushboolean(L1, getnum); }
        else if
            EQ("pushcclosure") { lua_pushcclosure(L1, testC, getnum); }
        else if
            EQ("pushint") { lua_pushinteger(L1, getnum); }
        else if
            EQ("pushnil") { lua_pushnil(L1); }
        else if
            EQ("pushnum") { lua_pushnumber(L1, (lua_Number)getnum); }
        else if
            EQ("pushstatus") { pushcode(L1, status); }
        else if
            EQ("pushstring") { lua_pushstring(L1, getstring); }
        else if
            EQ("pushupvalueindex") { lua_pushinteger(L1, lua_upvalueindex(getnum)); }
        else if
            EQ("pushvalue") { lua_pushvalue(L1, getindex); }
        else if
            EQ("pushfstringI") { lua_pushfstring(L1, lua_tostring(L, -2), (int)lua_tointeger(L, -1)); }
        else if
            EQ("pushfstringS") { lua_pushfstring(L1, lua_tostring(L, -2), lua_tostring(L, -1)); }
        else if
            EQ("pushfstringP") { lua_pushfstring(L1, lua_tostring(L, -2), lua_topointer(L, -1)); }
        else if
            EQ("rawget")
            {
                int t = getindex;
            ...
            EQ("remove") { lua_remove(L1, getnum); }
        else if
            EQ("replace") { lua_replace(L1, getindex); }
        else if
            EQ("resume")
            {
                int i = getindex;
            ...
            EQ("settable") { lua_settable(L1, getindex); }
        else if
            EQ("settop") { lua_settop(L1, getnum); }
        else if
            EQ("testudata")
            {
                int i = getindex;
            ...
            EQ("tobool") { lua_pushboolean(L1, lua_toboolean(L1, getindex)); }
        else if
            EQ("tocfunction") { lua_pushcfunction(L1, lua_tocfunction(L1, getindex)); }
        else if
            EQ("tointeger") { lua_pushinteger(L1, lua_tointeger(L1, getindex)); }
        else if
            EQ("tonumber") { lua_pushnumber(L1, lua_tonumber(L1, getindex)); }
        else if
            EQ("topointer") { lua_pushlightuserdata(L1, cast_voidp(lua_topointer(L1, getindex))); }
        else if
            EQ("touserdata") { lua_pushlightuserdata(L1, lua_touserdata(L1, getindex)); }
        else if
            EQ("tostring")
            {
            ...
            EQ("type") { lua_pushstring(L1, luaL_typename(L1, getnum)); }
        else if
            EQ("xmove")
            {
                int f = getindex;
            ...
            EQ("yield") { return lua_yield(L1, getnum); }
        else if
            EQ("yieldk")
            {
                int nres = getnum;
            ...
            EQ("toclose") { lua_toclose(L1, getnum); }
        else
