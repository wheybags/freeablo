...
namespace Level
{
    ...
    int32_t& get(int32_t x, int32_t y, Dun& dun) { return dun.mBlocks[x + y * dun.width()]; }

    const int32_t& get(int32_t x, int32_t y, const Dun& dun) { return dun.mBlocks[x + y * dun.width()]; }

    Misc::Helper2D<Dun, int32_t&> Dun::operator[](int32_t x) { return Misc::Helper2D<Dun, int32_t&>(*this, x, get); }

    Misc::Helper2D<const Dun, const int32_t&> Dun::operator[](int32_t x) const { return Misc::Helper2D<const Dun, const int32_t&>(*this, x, get); }

    int32_t Dun::width() const { return mWidth; }

    int32_t Dun::height() const { return mHeight; }
}
