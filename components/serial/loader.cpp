...
namespace Serial
{
    ...
    {
        return mStream.read_bool();
    }

    template <> int64_t Loader::load<int64_t>()
    {
        return mStream.read_int64_t();
    }

    template <> uint64_t Loader::load<uint64_t>()
    {
        return mStream.read_uint64_t();
    }


    template <> int32_t Loader::load<int32_t>()
    {
        return mStream.read_int32_t();
    }

    template <> uint32_t Loader::load<uint32_t>()
    {
        return mStream.read_uint32_t();
    }

    template <> int16_t Loader::load<int16_t>()
    {
        return mStream.read_int16_t();
    }

    template <> uint16_t Loader::load<uint16_t>()
    {
        return mStream.read_uint16_t();
    }

    template <> int8_t Loader::load<int8_t>()
    {
       return mStream.read_int8_t();
    }

    template <> uint8_t Loader::load<uint8_t>()
    {
        return mStream.read_uint8_t();
    }

    template <> std::string Loader::load<std::string>()
    {
        return mStream.read_string();
    }

    void Loader::startCategory(const std::string& name)
    {
        mStream.startCategory(name);
    }

    void Loader::endCategory(const std::string& name)
    {
        mStream.endCategory(name);
    }
    ...
    void Saver::startCategory(const std::string& name)
    {
        mStream.startCategory(name);
    }

    void Saver::endCategory(const std::string& name)
    {
        mStream.endCategory(name);
    }

    template<typename T>
    ScopedCategory<T>::ScopedCategory(std::string&& name, T& saverOrLoader)
        : mName(std::move(name))
        , mSaverOrLoader(saverOrLoader)
    {
        mSaverOrLoader.startCategory(mName);
    }

    template<typename T>
    ScopedCategory<T>::~ScopedCategory()
    {
        mSaverOrLoader.endCategory(mName);
    }
