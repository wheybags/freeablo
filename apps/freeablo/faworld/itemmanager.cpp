...
namespace FAWorld
{
    ...
    uint32_t ItemManager::getUniqueItemCode() { return ++lastUnique; }
    ...
    DiabloExe::BaseItem& ItemManager::getBaseItemByUniqueCode(uint8_t uniqCode) { return mUniqueCodeToBaseItem.find(uniqCode)->second; }
    ...
    void ItemManager::putItemOnFloor(Item& item, std::pair<size_t, size_t> floor_pos)
    {
        ...
        std::map<ItemPosition, Item>::const_iterator it = std::find_if(
            mItemPositionMap.begin(), mItemPositionMap.end(), [&](const std::pair<ItemPosition, Item>& pair) -> bool { return pair.second == item; });
    ...
    std::pair<size_t, size_t> ItemPosition::getFloorPosition() const { return this->mFloorPosition; }

    void ItemPosition::setFloorPosition(std::pair<size_t, size_t> pos) { this->mFloorPosition = pos; }

    ItemPosition::ItemPosition(std::pair<size_t, size_t> floorPosition) { this->mFloorPosition = floorPosition; }

    bool ItemPosition::operator<(const ItemPosition rhs) const { return mFloorPosition < rhs.mFloorPosition; }
