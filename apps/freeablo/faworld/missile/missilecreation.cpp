...
namespace FAWorld
{
    namespace Missile
    {
        ...
        void MissileCreation::singleFrame16Direction(Missile& missile, Misc::Point dest)
        {
            ...
            missile.mGraphics.push_back(boost::make_unique<MissileGraphic>("", missile.getGraphicsPath(0), direction16, srcPos));
        }
        ...
        void MissileCreation::animated16Direction(Missile& missile, Misc::Point dest)
        {
            ...
            int32_t direction16 = static_cast<int32_t>(direction.getDirection16());
            srcPos.setSpeed(3536);
            missile.mGraphics.push_back(boost::make_unique<MissileGraphic>("", missile.getGraphicsPath(direction16), nonstd::nullopt, srcPos));
        }
            ...
            for (auto angleOffset : {-90, 90})
            {
                ...
                for (int32_t i = 0; i < 6; i++)
                {
                    missile.mGraphics.push_back(
                        boost::make_unique<MissileGraphic>(missile.getGraphicsPath(0), missile.getGraphicsPath(1), nonstd::nullopt, Position(point)));
        ...
        {
            missile.mGraphics.push_back(boost::make_unique<MissileGraphic>("", missile.getGraphicsPath(0), nonstd::nullopt, Position(missile.mSrcPoint)));
        }
    }
}
